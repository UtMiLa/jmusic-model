...
describe('Queries', () => {
    it('SelectionAll should select all elements', () => {
        ...
        expect(select.isSelected({} as Model, { elementNo: 9999, staffNo: -8, voiceNo: 500 })).to.be.true;
    });
    ...
    it('should select all elements in one voice', () => {
        ...
        const select = new SelectionVoiceTime(1, 0, Time.StartTime, Time.EternityTime);

        const isSelected = (elem: ElementIdentifier) => select.isSelected(new JMusic(model), elem);
    ...
    it('should select all elements in one voice', () => {
        ...
        const select = new SelectionVoiceTime(1, 0, Time.newAbsolute(7, 8), Time.newAbsolute(5, 4));
