...
function multiSeqVoicesToSingleSeqVoices(voices: VoiceDef[], vars: VariableRepository): VoiceDef[] {
    ...
    const f = (cnt: VoiceDef, idx: number) => new MultiFlexibleSequence(cnt.contentDef, vars).seqs.map(seq => ({
        contentDef: seq.asObject,
        noteDirection: voices.length === 1 ? NoteDirection.Undefined : idx % 2 === 0 ? NoteDirection.Up : NoteDirection.Down
    } as VoiceDef));
