...
import { varDictActiveToDef } from '../score/variables';




export function convertVarDataToActive(vars: VarDictDef): ActiveVarRepo {
    return R.map((value: FlexibleItem) => convertSequenceDataToActive(flexibleItemToDef(value), vars), vars);
}

export function convertVarActiveToData(vars: ActiveVarRepo): VarDictDef {
...
export function convertProjectActiveToData(activeProject: ActiveProject): ProjectDef {
    return {
        ...
        vars: varDictActiveToDef(activeProject.vars)
    };
}
